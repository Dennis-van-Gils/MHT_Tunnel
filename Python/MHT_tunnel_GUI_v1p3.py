...
PENS = [None] * 12
params = {'width': 2}
PEN_WHITE = pg.mkPen(color=[255, 255, 255], **params)
...
class MainWindow(QtWid.QWidget):
    def __init__(self, parent=None, **kwargs):
        ...
        self.setWindowTitle("MHT tunnel control v1.3")
        ...
        grid.addWidget(QtWid.QLabel("%s2.2 %s" % (CHAR_PM, CHAR_DEG_C)), 0, 1)
        ...
        self.chkbs_tunnel_temp = [
                ...
                QtWid.QCheckBox("Ambient"      , **p, checked=True),
        ...
        self.ambient_temp           = QtWid.QLineEdit(**params, readOnly=True)
        ...
        grid.addWidget(self.chkbs_tunnel_temp[4]  , 5, 0)
        grid.addWidget(self.tunnel_outlet_temp    , 1, 1)
        grid.addWidget(self.tunnel_inlet_temp     , 2, 1)
        grid.addWidget(self.ambient_temp          , 3, 1)
        grid.addWidget(self.chiller_read_temp     , 4, 1)
        grid.addWidget(self.chiller_read_setpoint , 5, 1)
        grid.addWidget(QtWid.QLabel("%s0.03 %s" % (CHAR_PM, CHAR_DEG_C)), 1, 2)
        grid.addWidget(QtWid.QLabel("%s0.03 %s" % (CHAR_PM, CHAR_DEG_C)), 2, 2)
        grid.addWidget(QtWid.QLabel("%s0.03 %s" % (CHAR_PM, CHAR_DEG_C)), 3, 2)
        grid.addWidget(QtWid.QLabel("%s0.10 %s" % (CHAR_PM, CHAR_DEG_C)), 4, 2)
        ...
        self.CH_ambient = ChartHistory(
                C.CH_SAMPLES_PT104, self.pi_tunnel_temp.plot(pen=PEN_WHITE))
        ...
        legend.addItem(self.CH_ambient.curve         , name="ambient")
